...
class SupabaseSyncManager {
  ...
  private activeHydrationUserId: string | null = null;
  ...
  public setCurrentUser(userId: string | null, email: string | null = null) {
    ...
    if (!userId) {
      ...
      this.scheduleBackgroundHydration(userId);
    }
  }

  /**
   * Cancels any pending scheduled background hydration.
   */
  public cancelPendingHydration() {
    if (this.pendingHydrationTimer) {
      if (
        this.pendingHydrationTimer.type === "idle" &&
        typeof window !== "undefined" &&
        "cancelIdleCallback" in window
      ) {
        (window as any).cancelIdleCallback(this.pendingHydrationTimer.handle);
      } else if (this.pendingHydrationTimer.type === "timeout") {
        clearTimeout(this.pendingHydrationTimer.handle);
      }
      this.pendingHydrationTimer = null;
    }
    this.pendingHydrationUserId = null;
  }

  /**
   * Schedules full cloud migration and hydration asynchronously in the background.
   * Ensures auth and profile painting remain instantaneous without blocking on data sync.
   * Enforces an in-flight guard: exactly one active background hydration per user ID.
   */
  public scheduleBackgroundHydration(userId: string) {
    if (!userId) return;

    // In-flight guard: if already scheduled or currently running for this user, do not start another copy
    if (this.pendingHydrationUserId === userId || this.activeHydrationUserId === userId) {
      return;
    }

    // Cancel any pending hydration for a different user
    this.cancelPendingHydration();
    this.pendingHydrationUserId = userId;

    const run = async () => {
      this.pendingHydrationTimer = null;
      this.pendingHydrationUserId = null;

      // Invalidate if user changed or signed out while the job was waiting
      if (this.currentUserId !== userId) {
        return;
      }

      if (this.activeHydrationUserId === userId) {
        return;
      }

      this.activeHydrationUserId = userId;
      try {
        await this.migrateAndHydrateUser(userId);
      } catch (err) {
        console.warn("[SyncService] Background hydration error:", err);
      } finally {
        if (this.activeHydrationUserId === userId) {
          this.activeHydrationUserId = null;
        }
    ...
    if (typeof window !== "undefined" && "requestIdleCallback" in window) {
      const handle = (window as any).requestIdleCallback(run, { timeout: 1500 });
      this.pendingHydrationTimer = { type: "idle", handle };
    } else {
      const handle = setTimeout(run, 50);
      this.pendingHydrationTimer = { type: "timeout", handle };
    }
  }
