 ...
 *
 * Tests:
 *  1. Production canonical URL never resolves to localhost:3000.
 *  2. Local development canonical URL correctly uses localhost:3000 in node server context.
 *  3. Canonical callback URL generation is correct.
 *  4. Internal redirect sanitization works as intended.
...
describe("OAuth & Canonical Origin Resolution", () => {
  ...
  it("6. Generates canonical authentication callback URL for verification and sign-in", () => {
    (process.env as Record<string, string | undefined>).NODE_ENV = "production";
    process.env.NEXT_PUBLIC_SITE_URL = "https://reec.academy";

    const callbackDefault = getAuthCallbackUrl("/");
    expect(callbackDefault).toBe("https://reec.academy/auth/callback");

    const callbackWithNext = getAuthCallbackUrl("/lesson/phase-01/week-03/day-02");
