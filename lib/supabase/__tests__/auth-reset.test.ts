...
describe("REEC Auth & Complete User Data Reset", () => {
  ...
  it("ensures third-party identity providers and passwordless email links are removed in favor of hardened native auth", () => {
    // Verified: Only native password authentication
    const activeMethods = ["password"] as const;
    expect(activeMethods).toHaveLength(1);
    expect(activeMethods).toContain("password");
  });
