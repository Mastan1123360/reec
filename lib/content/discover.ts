...
export async function getHiddenLessonTriggers(): Promise<HiddenLessonTriggerRegistryEntry[]> {
  ...
  // Merge triggers directly registered in the trigger service (e.g. in test or client mode)
  try {
    const { HiddenLessonTriggerService } = await import("@/lib/hidden-lessons/service");
    for (const item of HiddenLessonTriggerService.getAllTriggers()) {
      if (!triggerMap.has(item.hiddenLessonId)) {
        triggerMap.set(item.hiddenLessonId, {
          hiddenLessonId: item.hiddenLessonId,
          slug: item.slug || item.hiddenLessonId.split("-").pop()?.toLowerCase() || "nll",
          title: item.title,
          subtitle: item.subtitle ?? null,
          description: item.description ?? null,
          badge: item.badge || "NLL",
          tags: item.tags || [],
          sourceLessonId: item.trigger.lessonId || null,
...
export async function searchLessons(query: string, unlockedHiddenIds: string[] = []): Promise<Lesson[]> {
  ...
  if (unlockedHiddenIds.length > 0) {
    ...
    try {
      const { HiddenLessonTriggerService } = await import("@/lib/hidden-lessons/service");
      for (const reg of HiddenLessonTriggerService.getAllTriggers()) {
        const idLower = reg.hiddenLessonId.toLowerCase();
        if (normalizedUnlocked.has(idLower) && !pool.some((p) => p.frontmatter.id?.toLowerCase() === idLower)) {
          pool.push({
            slug: ["hidden-lessons", reg.slug || "nll"],
            path: `/hidden-lessons/${reg.slug || "nll"}`,
            frontmatter: {
              id: reg.hiddenLessonId,
              phase: 1,
              week: null,
              day: null,
              title: reg.title,
              ...
              badge: reg.badge || "NLL",
              slug: reg.slug || "nll",
              trigger: reg.trigger ?? null,
            },
            sections: [],
            blocks: [],
            excerpt: reg.description || reg.title,
