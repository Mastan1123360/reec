...
export function HeaderControls({
  ...
  return (
    <div className="flex items-center gap-1.5 sm:gap-2.5">
      {/* Mobile Compact Search Button (< sm) */}
      <button
        onClick={onSearchOpen}
        aria-label="Search curriculum"
        className="sm:hidden relative flex h-8 w-8 items-center justify-center rounded-xl glass-control text-slate-600 dark:text-slate-300 transition-all hover:text-slate-900 dark:hover:text-slate-100 hover:border-slate-300 dark:hover:border-white/20 active:scale-95 cursor-pointer shrink-0"
      >
        <Search size={14} className="text-slate-500 dark:text-slate-400" />
      </button>

      {/* Search Pill Input with Apple Glass (sm+) */}
      <button
        onClick={onSearchOpen}
        className="hidden sm:flex group w-44 md:w-52 lg:w-60 items-center gap-2 rounded-xl glass-control px-3 py-1.5 text-xs text-slate-500 dark:text-slate-400 transition-all hover:border-blue-500/40 dark:hover:border-white/20 hover:text-slate-800 dark:hover:text-slate-200 active:scale-[0.98] shrink-0 cursor-pointer"
      >
        <Search
          size={13}
        ...
        />
        <span className="text-left font-normal text-xs truncate">Search...</span>
        <kbd className="ml-auto rounded-md bg-slate-200/50 dark:bg-white/[0.08] px-1.5 py-0.5 text-[9px] font-mono text-slate-500 dark:text-slate-400 border border-slate-900/[0.06] dark:border-white/[0.06]">
          ⌘K
        </kbd>
      </button>
...
function getRouteLabel(pathname: string): { title: string; subtitle?: string } {
  ...
  if (pathname.startsWith("/hidden-lessons")) return { title: "Hidden Lessons", subtitle: "Declassified Compiler Failure Labs" };
...
export function AppHeader({
  ...
  return (
    <header
      className="h-14 lg:h-16 shrink-0 flex items-center justify-between px-4 sm:px-6 lg:px-8 border-b border-slate-900/[0.08] dark:border-white/[0.08] bg-white/80 dark:bg-[#070914]/75 backdrop-blur-[36px] backdrop-saturate-[160%] z-20"
      style={{
