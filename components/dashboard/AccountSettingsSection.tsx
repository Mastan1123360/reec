      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        ...
        >
          <div className="flex items-start gap-3">
            <div
              className={cn(
                "w-9 h-9 rounded-full flex items-center justify-center text-white shrink-0 shadow-xs",
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center justify-between gap-1">
                ...
                {user && (
                  <span className="text-[9.5px] font-mono px-1.5 py-0.2 rounded bg-blue-500/10 text-blue-600 dark:text-blue-400 font-semibold">
                    @{username || user.user_metadata?.username || user.email?.split("@")[0]}
                  </span>
                )}
              </div>
              <p className="text-xs sm:text-[13px] font-bold text-slate-900 dark:text-slate-100 truncate mt-0.5">
                {user?.user_metadata?.display_name || user?.email || "Guest Session"}
              </p>
              <span className="text-[11px] text-slate-500 dark:text-slate-400 block mt-0.5 truncate">
                {user ? user.email : "Local Browser Persistence"}
              </span>
            </div>
          </div>

          <div className="mt-3 pt-3 border-t border-slate-900/[0.04] dark:border-white/[0.05] flex items-center justify-between gap-2">
            {user ? (
              <>
                <button
                  type="button"
                ...
                <button
                  type="button"
                  onClick={() => signOut()}
                  className="text-xs font-semibold text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-200 transition-colors cursor-pointer"
                >
                  Sign Out
                </button>
              </>
              ...
              <button
                ...
                className="text-xs font-bold px-3 py-1.5 rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white shadow-xs transition-all cursor-pointer"
              >
                Sign In / Register
              </button>
            )}
        ...
        >
          <div className="flex items-start gap-3">
            ...
            <div className="min-w-0">
              ...
              <p className="text-xs sm:text-[13px] font-bold text-slate-900 dark:text-slate-100 truncate mt-0.5">
                {user ? "Email & Password" : "Not Signed In"}
              </p>
              <span className="text-[11px] text-slate-500 dark:text-slate-400 block mt-0.5">
                Hardened Password Authentication
              </span>
            </div>
          </div>
          ...
          <div className="mt-3 pt-3 border-t border-slate-900/[0.04] dark:border-white/[0.05] flex items-center justify-between">
            <span className="text-[11px] text-slate-600 dark:text-slate-300">
              {user ? "Protected by Cloud Auth" : "Sign In to Protect Data"}
            </span>
            {!user && (
              <button
                type="button"
