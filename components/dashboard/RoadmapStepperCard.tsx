...
export function RoadmapStepperCard({ phases }: RoadmapStepperCardProps) {
  ...
  // Hover & 3-second idle cooldown management for navigation arrows
  const [isHovered, setIsHovered] = React.useState(false);
  const [isIdle, setIsIdle] = React.useState(true);
  const idleTimerRef = React.useRef<NodeJS.Timeout | null>(null);

  const resetCooldownTimer = React.useCallback(() => {
    setIsHovered(true);
    setIsIdle(false);
  ...
  const showArrows = isHovered && !isIdle;
  ...
  // Non-carousel linear scroll navigation
  ...
  return (
    ...
    >
      ...
      {/* Navigation Track with Hover-Only 3s Cooldown Arrows */}
      <div className="relative group/track">
        {/* Floating Left Arrow Button - appears only when pointer is on section and not idle (3s cooldown) */}
        <button
          type="button"
          ...
          className={cn(
            "absolute -left-2.5 top-1/2 -translate-y-1/2 z-30 flex h-7 w-7 sm:h-8 sm:w-8 items-center justify-center rounded-full glass-control transition-all duration-300 shadow-md",
            showArrows && canScrollLeft
              ? "opacity-100 pointer-events-auto hover:scale-110 active:scale-95 text-slate-800 dark:text-white hover:text-blue-600 dark:hover:text-blue-400 cursor-pointer"
        ...
        {/* Floating Right Arrow Button - appears only when pointer is on section and not idle (3s cooldown) */}
        <button
          type="button"
          ...
          title="Next phase"
          className={cn(
            "absolute -right-2.5 top-1/2 -translate-y-1/2 z-30 flex h-7 w-7 sm:h-8 sm:w-8 items-center justify-center rounded-full glass-control transition-all duration-300 shadow-md",
        ...
        {/* Horizontal linear track - non-carousel, smooth linear scrolling */}
        <div
          ref={scrollContainerRef}
          tabIndex={0}
          className="flex gap-2.5 overflow-x-auto pb-1 scroll-smooth -mx-1 px-2 focus-visible:outline-none scrollbar-none"
          style={{
            scrollbarWidth: "none",
          ...
          {phases.map((phase) => {
            ...
            return (
              <Link
                ...
                className="group relative shrink-0 w-[205px] xl:w-[220px] rounded-xl glass-elevated p-3 flex flex-col justify-between transition-all duration-300 text-left hover:-translate-y-0.5"
              >
