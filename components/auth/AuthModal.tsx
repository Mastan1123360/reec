...
export function AuthModal() {
  ...
  const [blockedPopupUrl, setBlockedPopupUrl] = useState<{ provider: string; url: string } | null>(null);
  ...
  // Synchronize OAuth events (cancellation, window closed, errors)
  useEffect(() => {
    const handleCancel = () => setOauthLoading(null);
    const handleClosed = () => setOauthLoading(null);
    const handleError = (e: CustomEvent) => {
      setOauthLoading(null);
      if (e.detail) setError(String(e.detail));
    };

    window.addEventListener("reec_oauth_cancel", handleCancel);
    window.addEventListener("reec_oauth_popup_closed", handleClosed);
    window.addEventListener("reec_oauth_error", handleError as EventListener);

    return () => {
      window.removeEventListener("reec_oauth_cancel", handleCancel);
      window.removeEventListener("reec_oauth_popup_closed", handleClosed);
      window.removeEventListener("reec_oauth_error", handleError as EventListener);
    };
  }, []);
  ...
  const handleOAuth = async (provider: "google" | "github") => {
    ...
    try {
      ...
      if (res?.error) {
        setOauthLoading(null);
        if (res.error.message.toLowerCase().includes("not configured")) {
          setShowSetupGuide(true);
        } else {
          setError(res.error.message || `Failed to sign in with ${provider}.`);
        }
      } else if (res?.popupBlocked && res?.url) {
        setOauthLoading(null);
        setBlockedPopupUrl({ provider, url: res.url });
      }
      // If popup successfully opened, keep oauthLoading active until callback or popup close
    } catch (err: unknown) {
      setOauthLoading(null);
  ...
  const isAnyLoading = isSubmitting || Boolean(oauthLoading);
      ...
      {isAuthModalOpen && (
        ...
        >
          ...
          >
            ...
            {/* Social OAuth Options (Available for both Sign In and Sign Up) */}
            <div className="space-y-2.5 mb-3">
              ...
              {/* Popup Blocked Warning Notice */}
              {blockedPopupUrl && (
                <div className="p-2.5 rounded-xl bg-sky-500/10 border border-sky-500/20 text-sky-800 dark:text-sky-300 text-xs flex items-center justify-between gap-2">
                  <span className="text-[11px] leading-tight">Pop-up window blocked by browser.</span>
                  <button
                    type="button"
                    onClick={() => {
                      window.open(blockedPopupUrl.url, "_blank");
                      setBlockedPopupUrl(null);
                    }}
                    className="flex items-center gap-1 px-2 py-1 rounded-md bg-sky-600 hover:bg-sky-500 text-white font-medium text-[11px] shrink-0 cursor-pointer"
                  >
              ...
              {/* Supabase Configuration Notice if Credentials Missing */}
              {showSetupGuide && (
                <div className="p-3 rounded-xl bg-amber-500/10 border border-amber-500/20 text-xs text-amber-900 dark:text-amber-200 space-y-2">
                  ...
                  <p className="text-[11px] text-slate-600 dark:text-slate-300 leading-relaxed">
                    To enable Google & GitHub sign-in, add your Supabase credentials to environment variables and whitelist this Redirect URL:
                  </p>
                  <div className="flex items-center gap-1.5 p-1.5 rounded-lg bg-black/20 font-mono text-[10px] break-all">
                    ...
                    <button
                      ...
                      className="flex items-center gap-1 px-2 py-0.5 rounded bg-amber-500/20 hover:bg-amber-500/30 text-amber-800 dark:text-amber-200 font-semibold shrink-0 cursor-pointer transition-colors"
                    >
