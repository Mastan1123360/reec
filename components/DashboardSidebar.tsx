...
export function DashboardSidebar({
  ...
  const { currentAvatar } = useUserAvatar(profile?.avatarId, profile?.gender);
  const displayName = user ? (profile?.displayName || "User") : "guest";
  const userHandle = profile?.username || username || "learner";
  ...
  return (
    ...
    >
      ...
      {/* 4. Bottom System Status & Collapse Control (Clean, un-cramped dock) */}
      <div className="p-2.5 w-full shrink-0 flex flex-col gap-2 border-t border-slate-900/[0.05] dark:border-white/[0.06]">
        <div
          className={cn(
            "flex items-center rounded-xl transition-all duration-200 glass-elevated",
            collapsed ? "justify-center p-2" : "px-2.5 py-2 gap-2.5"
          )}
          title={collapsed ? `${displayName} (${user ? `@${userHandle}` : "Guest"})` : undefined}
        >
          <div
            className={cn(
              "flex h-7 w-7 shrink-0 items-center justify-center rounded-full text-white font-black text-xs shadow-xs",
          ...
          >
            {user ? currentAvatar.svgIcon("w-full h-full") : "g"}
          </div>
          ...
          {!collapsed && (
            <button
              ...
              className="flex h-7 w-7 items-center justify-center rounded-lg glass-control text-slate-400 hover:text-slate-800 dark:text-slate-500 dark:hover:text-slate-200 hover:border-slate-300 dark:hover:border-white/20 transition-all active:scale-95 cursor-pointer shrink-0"
